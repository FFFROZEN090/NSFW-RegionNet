...
class VisualizationUtils:
    ...
    def draw_keypoints(
        ...
        for i, keypoint in enumerate(detection.keypoints):
            if keypoint.score > 0.3:  # Only draw confident keypoints
                ...
                cv2.circle(img_copy, (keypoint.x, keypoint.y), keypoint_radius + 2, color, confidence_thickness)
    ...
    @staticmethod
    def _draw_skeleton(image: np.ndarray, detection: DetectionResult, line_thickness: int) -> np.ndarray:
    ...
    def draw_prompts(
        ...
        for point, label in zip(points, labels):
            ...
            if label == 1:  # Positive prompt
                ...
                cv2.putText(img_copy, '+', (x-3, y+3), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    def create_detection_summary(
        ...
        text_y = 30
        cv2.putText(
            result_img, f"Detected {len(detections)} person(s)",
    ...
    def create_pipeline_visualization(
        ...
        # 2. Bounding box
        bbox_img = VisualizationUtils.draw_bounding_box(original_image, detection)
        visualizations['bounding_box'] = bbox_img
        
        # 3. SAM2 prompts
        prompts_img = VisualizationUtils.draw_prompts(original_image, prompts_points, prompts_labels)
        ...
        combined_img = VisualizationUtils.draw_keypoints(combined_img, detection, draw_labels=False)
        combined_img = VisualizationUtils.draw_prompts(combined_img, prompts_points, prompts_labels)
        if chest_mask is not None:
            combined_img = VisualizationUtils.draw_mask_overlay(combined_img, chest_mask, alpha=0.3)
