...
class Keypoint(BaseModel):
    ...
    score: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score of the keypoint"
    )
    label: str = Field(
        ..., description="Label of the keypoint (e.g., 'nose', 'left_shoulder')"
    )
...
class DetectionResult(BaseModel):
    ...
    keypoints: List[Keypoint] = Field(
        default_factory=list, description="List of detected keypoints"
    )
    confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall detection confidence"
    )
...
class PipelineResult(BaseModel):
    ...
    original_image: Optional[np.ndarray] = Field(
        default=None, description="Original input image"
    )
    image_shape: Tuple[int, int, int] = Field(
        ..., description="Shape of the input image (H, W, C)"
    )
    detection_results: List[DetectionResult] = Field(
        default_factory=list, description="Person detection results"
    )

    # Mask results
    skin_mask: Optional[np.ndarray] = Field(
        default=None, description="Skin segmentation mask from SAM2"
    )
    chest_area_mask: Optional[np.ndarray] = Field(
        default=None, description="Chest area mask from geometry"
    )
    final_exposed_mask: Optional[np.ndarray] = Field(
        default=None, description="Final exposed area mask"
    )

    # Analysis results
    is_exposed: bool = Field(
        default=False, description="Whether exposed chest area is detected"
    )
    exposed_area: float = Field(
        default=0.0, ge=0.0, description="Area of exposed region in pixels"
    )
    exposure_ratio: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="Ratio of exposed area to total chest area",
    )
    ...
    class Config:
        ...
        json_encoders = {np.ndarray: lambda v: v.tolist() if v is not None else None}
