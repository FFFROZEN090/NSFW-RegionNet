...
class ChestExposurePipeline:
    ...
    def _initialize_models(self) -> None:
        ...
        try:
            ...
            sam2_model_type = self.config['models'].get('sam2_model_type', 'hiera_large')
    ...
    def _process_single_detection(
        ...
        print(f"  Generated {len(points)} prompt points ({np.sum(labels)} positive, {np.sum(1-labels)} negative)")
    ...
    def _save_detection_visualizations(self, image: np.ndarray, result: Dict[str, Any], output_dir: str) -> None:
        ...
        triangle_img = self.visualizer.draw_mask_overlay(image.copy(), triangle_mask, color=(0, 255, 255))
        cv2.imwrite(os.path.join(output_dir, 'chest_triangle.png'), triangle_img)
        
        # 5. SAM2 segmentation result
        if mask is not None:
            segmentation_img = self.visualizer.draw_mask_overlay(image.copy(), mask, color=(0, 255, 0))
            cv2.imwrite(os.path.join(output_dir, 'sam2_segmentation.png'), segmentation_img)
    ...
    def _create_summary_visualization(self, image: np.ndarray, results: Dict[str, Any], output_dir: str) -> None:
    ...
    def process_batch(self, input_dir: str, output_dir: str = None) -> List[Dict[str, Any]]:
        ...
        print(f"Batch processing complete. Processed {len(all_results)}/{len(image_files)} images")
        return all_results
    ...
    def get_pipeline_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sam_segmenter': self.sam_segmenter.get_model_info() if self.sam_segmenter else None,
            'models_initialized': self.yolo_detector is not None and self.sam_segmenter is not None
        }