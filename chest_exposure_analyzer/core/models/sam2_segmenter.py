...
class SamSegmenter:
    ...
    def __init__(self, model_path: str, model_type: str = 'hiera_large', device: str = 'cuda'):
        """
    ...
    def _load_model(self) -> None:
        ...
        if not os.path.exists(self.model_path):
            ...
            print("Please download SAM2 checkpoint from: https://github.com/facebookresearch/segment-anything-2")
            self.predictor = None
            return
    ...
    def segment(self, image: np.ndarray, points: np.ndarray, labels: np.ndarray) -> np.ndarray:
        """
        ...
        try:
            ...
            if len(masks) > 0:
                ...
                print(f"SAM2 segmentation complete. Best mask score: {scores[best_idx]:.3f}")
    ...
    def _generate_mock_mask(self, image: np.ndarray, points: np.ndarray, labels: np.ndarray) -> np.ndarray:
        """
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            'model_available': build_sam2 is not None and SAM2ImagePredictor is not None
        }