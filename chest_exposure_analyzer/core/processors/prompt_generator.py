...
class PromptGenerator:
    ...
    def _get_clothing_center(detection: DetectionResult) -> Optional[List[int]]:
        ...
        if (
            left_shoulder
            and right_shoulder
            and left_shoulder.score > 0.3
            and right_shoulder.score > 0.3
        ):
            ...
            if (
                left_hip
                and right_hip
                and left_hip.score > 0.3
                and right_hip.score > 0.3
            ):
    ...
    def _get_strategic_negative_points(
        ...
        if all_detections and max_points > 0:
            ...
            clothing_points = PromptGenerator._get_clothing_negative_points(
                primary_detection
            )
            if clothing_points:
        ...
        if remaining_slots > 0:
            background_point = PromptGenerator._get_strategic_background_point(
                all_detections, image_shape
            )
            if background_point:
        ...
        if remaining_slots > 0:
            corner_point = PromptGenerator._get_corner_background_point(
                all_detections, image_shape
            )
            if corner_point:
    ...
    def _calculate_forehead_position(detection: DetectionResult) -> Optional[List[int]]:
        ...
        if left_eye and right_eye and left_eye.score > 0.3 and right_eye.score > 0.3:
            ...
            forehead_y = eyes_center_y - int(
                abs(eyes_center_y - nose.y) * 0.8
            )  # 80% up from eyes toward forehead
    ...
    @staticmethod
    def _get_negative_points(
        all_detections: List[DetectionResult], image_shape: Tuple[int, int]
    ) -> List[List[int]]:
        """
        ...
        if len(negative_points) < 3:
            if len(all_detections) > 1:
                ...
                secondary_clothing = PromptGenerator._get_clothing_negative_points(
                    secondary_detection
                )
    ...
    @staticmethod
    def _get_strategic_background_point(
        all_detections: List[DetectionResult], image_shape: Tuple[int, int]
    ) -> Optional[List[int]]:
        """
    ...
    @staticmethod
    def _get_corner_background_point(
        all_detections: List[DetectionResult], image_shape: Tuple[int, int]
    ) -> Optional[List[int]]:
        """
        ...
        for position in corner_positions:
            if PromptGenerator._is_safe_background_point(
                position, all_detections, min_distance=120
            ):
                return position
    ...
    def _generate_safe_background_points(
        width: int, height: int, exclusion_zones: List[List[int]]
    ) -> List[List[int]]:
        """
    ...
    def _is_safe_background_point(
        point: List[int], all_detections: List[DetectionResult], min_distance: int = 100
    ) -> bool:
        """
        ...
        for detection in all_detections:
            ...
            bbox_distance = np.sqrt(
                (px - bbox_center_x) ** 2 + (py - bbox_center_y) ** 2
            )
    ...
    def _is_far_from_keypoints(
        point: List[int], detection: DetectionResult, min_distance: int = 30
    ) -> bool:
        """
    ...
    @staticmethod
    def _calculate_chest_center_point(
        detection: DetectionResult,
    ) -> Optional[List[int]]:
        """
        Calculate the chest center point based on shoulder keypoints.

        Args:
            detection: Detection result containing keypoints

        Returns:
            [x, y] coordinates of chest center point or None
        """
        left_shoulder = detection.get_keypoint_by_label("left_shoulder")
        right_shoulder = detection.get_keypoint_by_label("right_shoulder")

        # Both shoulders are required for chest center calculation
        if not (left_shoulder and right_shoulder):
            return None

        if not (left_shoulder.score > 0.3 and right_shoulder.score > 0.3):
            return None
        ...
        # Get additional keypoints for better chest center estimation
        neck = detection.get_keypoint_by_label("neck")  # If available
        left_hip = detection.get_keypoint_by_label("left_hip")
        right_hip = detection.get_keypoint_by_label("right_hip")

        # Strategy 1: Use neck and hips if available for more accurate positioning
        if (
            neck
            and neck.score > 0.3
            and left_hip
            ...
            hip_center_y = int((left_hip.y + right_hip.y) / 2)
            # Position chest center between neck and hips, closer to shoulders
            chest_center_y = int(neck.y + (hip_center_y - neck.y) * 0.4)

        # Strategy 2: Use hips without neck
        elif left_hip and right_hip and left_hip.score > 0.3 and right_hip.score > 0.3:

            hip_center_y = int((left_hip.y + right_hip.y) / 2)
            # Position chest center between shoulders and hips
            chest_center_y = int(
                shoulder_center_y + (hip_center_y - shoulder_center_y) * 0.5
            )

        # Strategy 3: Fallback to geometric estimation below shoulders
        else:
            # Position chest center below shoulders by a reasonable distance
            shoulder_width = abs(right_shoulder.x - left_shoulder.x)
            # Use shoulder width as reference for vertical offset (anatomical proportion)
            chest_offset = int(shoulder_width * 0.4)
            chest_center_y = shoulder_center_y + chest_offset

        return [shoulder_center_x, chest_center_y]
    ...
    def generate_chest_triangle_mask(
        ...
        Triangle is formed by chest center point and both shoulders.
        ...
        chest_center = PromptGenerator._calculate_chest_center_point(detection)
        left_shoulder = detection.get_keypoint_by_label("left_shoulder")
        right_shoulder = detection.get_keypoint_by_label("right_shoulder")

        if not all([chest_center, left_shoulder, right_shoulder]):
            return mask

        if not all([kp.score > 0.3 for kp in [left_shoulder, right_shoulder]]):
            return mask

        # Define triangle vertices: chest center + both shoulders
        triangle_points = np.array(
            [
                chest_center,  # Chest center point (calculated)
                [left_shoulder.x, left_shoulder.y],  # Left shoulder
                [right_shoulder.x, right_shoulder.y],  # Right shoulder
            ],
            dtype=np.int32,
        )
        ...
        triangle_points[:, 0] = np.clip(triangle_points[:, 0], 0, width - 1)
        triangle_points[:, 1] = np.clip(triangle_points[:, 1], 0, height - 1)
